...
add_filter( 'post_gallery', 'spine2_gallery', 10, 2 );
 ...
 */
if( ! function_exists( 'spine2_gallery' ) ){
function spine2_gallery( $output, $attr ) {
	...
	$output = "\n\t\t\t<ul id='gallery-{$id}-{$cleaner_gallery_instance}' class='col-{$columns} gallery gallery-{$id}'>";
	...
	foreach ( $attachments as $attachment ) {
		...
		/* Add the image. */
		$image = ( ( isset( $attr['link'] ) && 'file' == $attr['link'] ) ? wp_get_attachment_link( $attachment->ID, $size, false, false ) : wp_get_attachment_link( $attachment->ID, 'medium', true, false ) );
